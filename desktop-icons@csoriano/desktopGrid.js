...
var DesktopGrid = class {
    ...
    _onNewFolderClicked() {
        ...
        dialog.connect('response', (dialog, name) => {
            ...
            DBusUtils.NautilusFileOperationsProxy.CreateFolderRemote(dir.get_uri(),
                (result, error) => {
                    if (error)
                        throw new Error('创建文件夹时发生错误: ' + error.message);
                }
            );
        });
            ...
            (clipboard, text) => {
                ...
                if (is_cut) {
                    DBusUtils.NautilusFileOperationsProxy.MoveURIsRemote(files, desktopDir,
                        (result, error) => {
                            if (error)
                                throw new Error('移动文件时发生错误: ' + error.message);
                        }
                    );
                } else {
                    DBusUtils.NautilusFileOperationsProxy.CopyURIsRemote(files, desktopDir,
                        (result, error) => {
                            if (error)
                                throw new Error('复制文件时发生错误: ' + error.message);
                        }
                    );
                }
            }
    ...
    _doUndo() {
        DBusUtils.NautilusFileOperationsProxy.UndoRemote(
            (result, error) => {
                if (error)
                    throw new Error('撤销时发生错误: ' + error.message);
            }
        );
    }
    ...
    _doRedo() {
        DBusUtils.NautilusFileOperationsProxy.RedoRemote(
            (result, error) => {
                if (error)
                    throw new Error('恢复时发生错误: ' + error.message);
            }
        );
    }
        ...
        Gio.AppInfo.launch_default_for_uri_async(DesktopIconsUtil.getDesktopDir().get_uri(),
            ...
            (source, result) => {
                try {
                   ...
                   log('在文件管理器中打开桌面时发生错误: ' + e.message);
                }
            }
        );
    }
    ...
    _createDesktopBackgroundMenu() {
        let menu = new PopupMenu.PopupMenu(Main.layoutManager.dummyCursor,
                                           0, St.Side.TOP);
        menu.addAction(_("新建文件夹"), () => this._onNewFolderClicked());
        menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        this._pasteMenuItem = menu.addAction(_("粘贴"), () => this._onPasteClicked());
        this._undoMenuItem = menu.addAction(_("撤销"), () => this._onUndoClicked());
        this._redoMenuItem = menu.addAction(_("恢复"), () => this._onRedoClicked());
        menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        menu.addAction(_("在文件管理器中打开桌面"), () => this._onOpenDesktopInFilesClicked());
        menu.addAction(_("在控制台打开"), () => this._onOpenTerminalClicked());
        menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        menu.addSettingsAction(_("修改壁纸…"), 'gnome-background-panel.desktop');
        menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        menu.addSettingsAction(_("显示设置"), 'gnome-display-panel.desktop');
        menu.addSettingsAction(_("设置"), 'gnome-control-center.desktop');
    ...
    _getEmptyPlaceClosestTo(x, y, coordinatesAction) {
        ...
        if (!found)
            throw new Error(`显示器没有足够空间 ${this._bgManager._monitorIndex}`);
...
var RenamePopup = class {
    ...
    constructor(grid) {
        ...
        this._renameEntry = new St.Entry({ hint_text: _("输入文件名…"),
                                           can_focus: true,
                                           x_expand: true });
        this._renameEntry.clutter_text.connect('activate', this._onRenameAccepted.bind(this));
        this._renameOkButton= new St.Button({ label: _("确定"),
                                              style_class: 'app-view-control button',
                                              button_mask: St.ButtonMask.ONE | St.ButtonMask.THREE,
                                              reactive: true,
                                              can_focus: true,
                                              x_expand: true });
        this._renameCancelButton = new St.Button({ label: _("取消"),
                                                   style_class: 'app-view-control button',
                                                   button_mask: St.ButtonMask.ONE | St.ButtonMask.THREE,
                                                   reactive: true,
                                                   can_focus: true,
    ...
    _onRenameAccepted() {
        ...
        DBusUtils.NautilusFileOperationsProxy.RenameFileRemote(this._source.file.get_uri(),
            ...
            (result, error) => {
                if (error)
                    throw new Error('重命名文件时发生错误: ' + error.message);
            }
        );
    }
